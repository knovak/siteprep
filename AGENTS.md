...
directly by a user working interactively, so the whole point of one is that it
is *discoverable* - a skill an agent cannot see is a skill that never runs.
...
Codex look. A skill anywhere else is invisible.
...
- **The directory is flat.** Do not group skills into category subdirectories.
  ...
  nothing at the moment a skill is chosen, and nesting risks not being scanned
  at all.
...
- **Name a skill verb-first** - `deploy-demo`, `merge-prs`, `update-deck-events`.
  A flat list stays readable at thirty skills only if the names sort into
...
**A skill goes in `.claude/skills/` by default.** Putting one inside an
initiative's `skills/` is a deliberate choice, made by the user or stated by the
agent, and it means "this is meant for this initiative alone". Absent that
choice the skill is repo-wide, because a skill filed where nothing discovers it
cannot be invoked by the person it was written for.
...
modules. That is allowed and is not the same as the rule forbidding a *published
output* from referencing `initiatives/`: a skill is invoked deliberately by
someone who can see it fail, not served silently to a reader.
