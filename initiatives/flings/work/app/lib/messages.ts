...
Return exactly one JSON object with batch_id and revision copied from the manifest, and results as a list of objects containing delivery_id, status and evidence. Use only reported_sent, reported_failed, suppressed or unknown for status. Evidence is a short text observation or app message reference; never include a personal access link. If the manifest includes attempt, copy that number into each result as attempt. Uncertain outcomes stay unknown; omit deliveries you did not observe. Reports are claims, not verified recipient receipt.
