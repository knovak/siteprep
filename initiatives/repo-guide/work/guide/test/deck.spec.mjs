...
test('deck opens offline and supports complete keyboard navigation', async ({page}) => {
  ...
  await expect(page.locator('#progress')).toHaveText('1 / 20');

  await page.keyboard.press('ArrowRight');
  await expect(slides.nth(1)).toBeVisible();
  await expect(page.locator('#progress')).toHaveText('2 / 20');
  await page.keyboard.press('End');
  await expect(slides.nth(19)).toBeVisible();
  await expect(page.locator('#progress')).toHaveText('20 / 20');
