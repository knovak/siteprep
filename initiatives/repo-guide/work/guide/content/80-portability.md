...
publishes, the line between those two is pretty clean.
...
Four workflows schedule, validate, and publish. These shapes are read out of the
workflow files themselves, so what you see is what you'd actually be copying:
...
What to leave behind is everything about travel. The decks, the demos, and the
initiative histories already here are this repository's own subject matter, and
they're worth copying only if that subject is yours too.
...
The line between the way this repository works and what it publishes is pretty
...
carry the procedures the agents reach for by name. Both shapes are read out of
the files themselves, so this is what you'd really be taking.
