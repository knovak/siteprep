...
[Implementation plan, section 4](plan.html) remains the authoritative T1–T8 plan, together with its E1–E7 test extensions and the acceptance criteria in [spec.md](spec.html). No acceptance threshold is changed by adoption.
...
The supplied README's July 2026 results are preserved in [README.md](README.html). Those figures are historical reports, not test results from this adoption. The repository demo lacks `src/core.js`, `src/app.js`, `tools/build.py`, and the original `tests/` directory. Therefore its original unit, golden, performance, accessibility, cross-browser, and editorial suites cannot be rerun from this source snapshot. Recovering the development package is separate work if future changes require it.
...
Record observed outcomes in [log.md](log.html). These checks establish migration integrity and a browser smoke check; they do not replace the full inherited acceptance suite.
