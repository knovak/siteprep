...
so later evidence can reopen the right refinement without making one current
now. The earlier location-button idea is not included because the page already
provides **Show here** with an explicit, click-triggered permission request and
manual-entry fallback.

| Candidate | Why it could help | Likely size | Boundary and evidence |
|---|---|---:|---|
| **Replace the relative alternatives plot with a geographic coast map** | Makes islands, channels, borders, and station placement legible when several official coasts are plausible. | Medium | Keep the same candidates, thresholds, and closest-first result. Prefer local coastline data; disclose and attribute remote tiles if used. Reopen this when recorded confusion or wrong choices show that the relative plot is insufficient. |
...
official data can make the corresponding option actionable later.
