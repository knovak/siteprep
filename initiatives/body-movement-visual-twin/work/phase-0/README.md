...
1. Which body and anatomy assets may be packaged in an access-controlled hosted
   validation demo?
2. Can the full-body layers share one hierarchy while every declared
...
Phase 0 surface, rig and muscle paths project-authored. A concrete
...
`assets/original/reference-rig.json` is a procedural, anatomically unreviewed
full-body feasibility model generated by `scripts/build-reference-rig.mjs`:

- surface, skeleton and muscle paths use one hierarchy and animation clock;
- the 27-node rig includes the head, axial skeleton, shoulder girdle, arms,
  hands, pelvis, legs and feet, plus 40 paired superficial and deep muscle
  paths spanning the major displayed regions;
...
- every attachment is checked at every sample, producing 100 registration
  comparisons across the expanded fixture.
...
- `assets/original/reference-rig.json` — generated project-authored hierarchy, layers,
  motion samples, landmarks and review status.
- `scripts/build-reference-rig.mjs` — canonical full-body procedural rig and
  paired-muscle generator.
