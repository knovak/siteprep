...
The first useful version is a public, explorable 3D explanation of small
movements. It is not a pose catalog with anatomy painted on top: it should show
what moves beneath the surface while preserving the different teaching intent
of Feldenkrais, yoga, and the Alexander Technique.
...
5. **The movement collection is publishable, attributable, and correctable.**
   Every entry records its tradition, source, rights basis, authoring or review
   date, intended context, and any modification or contraindication. Only
   original, public-domain, openly licensed, or explicitly permitted material
   appears on the public site; a viewer can report an anatomical, movement, or
   attribution problem without changing the source record silently.
...
9. **It is a public website, not a workstation demo.** A first-time visitor can
   open an example without an account or special hardware, use the core camera
   and layer controls with pointer, keyboard, or touch, and understand what the
   controls reveal. Loading and fallback states remain usable on an ordinary
...
rights-cleared, cross-tradition movement library with an explorable layered
anatomy and an explicitly approximate visual twin.
...
- Changing body controls visibly changes only the dimensions the interface
  ...
  model.
- Every public movement and anatomical asset has a recorded rights basis and
  attribution, and removing one source does not break the movement data model.
...
hard boundary between assets that are technically accessible and assets or
lesson content the public site has a right to republish.

The final rights and trademark review is not an engineering judgement. It stays
as a separate legal blocker before the public collection can graduate, even if
the first build uses only apparently open material.
