...
import { matcherGroupsFor, sourceSlug } from './source-contract.mjs';
...
export function validatePrivateInventory(inventory) {
  ...
  for (const entry of inventory.sources) {
    if (!entry?.key || !entry.name || !SHAPES.has(entry.shape)) {
      throw new Error('private inventory: every source needs key, slug, name, and a supported shape');
    }
    if (keys.has(entry.key)) throw new Error(`private inventory: duplicate key ${entry.key}`);
    keys.add(entry.key);
    const slug = sourceSlug(entry);
    if (slugs.has(slug)) throw new Error(`private inventory: duplicate slug ${slug}`);
    slugs.add(slug);
