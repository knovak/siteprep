...
<body>
  ...
  ${judgeable ? `<dialog id="help-dialog" aria-labelledby="help-title">
    ...
    <p>Ask an assistant with this repository and connected Gmail to use <code>harvest-newsletter-stories</code>. Choose all configured sources or names from the list below, and give a date range or a number of recent days.</p>
    <blockquote>Use $harvest-newsletter-stories for all configured sources for the last 30 days, then refresh the existing private test site.</blockquote>
    <p>The <strong>store</strong> is your saved story collection, including its IDs, tags and locally imported judgments. The <strong>inventory</strong> is the separate configuration listing newsletters, Gmail search rules, extraction shapes and default date limits. The assistant uses the configured private files unless you supply other existing file paths.</p>
    <p>A harvest updates the local store and keeps a previous-generation backup. You can also request an offline review file, themes and event clusters, or a test-site refresh. These are optional: the example above requests the test refresh so the new stories appear here. “Last 30 days” includes today; explicit “through” dates include the end date. The assistant reports the exact dates, source counts, added or matched stories, flags and output paths.</p>
    <p>For all inputs, supported choices and examples, ask the assistant for the <code>harvest-newsletter-stories</code> usage guide. The <code>tag-newsletter-stories</code> skill handles requested themes and event clusters; <code>deploy-test</code> handles the requested test refresh. Refresh this page after the assistant finishes.</p>
