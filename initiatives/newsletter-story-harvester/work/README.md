...
| `test/` | One test per row of `test-plan.md` §4.1 | |

Not here, deliberately: every email, every model call, the page, and verdicts.
That is `plan.md` §3's phase 1 boundary, and the store deliberately knows nothing
about where a record came from beyond the fields the record carries.
