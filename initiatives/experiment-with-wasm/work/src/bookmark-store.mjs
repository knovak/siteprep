...
const SQL_BATCH_PARAMETERS = 100;

function sqlChunkSize(requestedSize, reservedParameters = 0) {
  return Math.max(1, Math.min(requestedSize, SQL_BATCH_PARAMETERS - reservedParameters));
}

export class WasmBookmarkStore {
  constructor(db, {ownerId = null, ownerEmail = null, batchSize = 100, idFactory = () => crypto.randomUUID()} = {}) {
    if (!db?.prepare || !db?.batch) throw new TypeError('A browser SQLite database binding is required');
    this.db = db;
  ...
  async canEditTemplates() { return true; }
  ...
  async applyKnownCaptureErrors(collectionId, urlKeys) {
    await this.assertCollectionWritable(collectionId);
    const urlBatchSize = sqlChunkSize(this.batchSize, 1);
  ...
  async updatedTagsFor(collectionId, ids) {
    ...
    for (const batch of chunks(ids, sqlChunkSize(this.batchSize, 4))) {
      const placeholders = batch.map(() => '?').join(', ');
  ...
  updatedTagStatements(collectionId, ids, stamp) {
    return chunks(ids, sqlChunkSize(this.batchSize, 4)).flatMap(batch => {
      const placeholders = batch.map(() => '?').join(', ');
      return [
  ...
  async applyVerdict(collectionId, {itemIds, verdict, at, sessionId, actionId}) {
    ...
    if (changes.length) {
      ...
      for (const batch of chunks(changes.map(change => change.item_id), sqlChunkSize(this.batchSize, 4))) {
        const placeholders = batch.map(() => '?').join(', ');
        statements.push(this.db.prepare(
  ...
  async applyTags(collectionId, {itemIds, tags, at, sessionId, actionId}) {
    ...
    if (changes.length) {
      ...
      for (const tag of wanted.filter(tag => !isUpdatedTag(tag))) {
        for (const batch of chunks(changedIds.filter(id => !existing.get(id).has(tag)), sqlChunkSize(this.batchSize, 4))) {
          if (!batch.length) continue;
          const placeholders = batch.map(() => '?').join(', ');
          statements.push(this.db.prepare(
            `INSERT OR IGNORE INTO tags (item_id, tag)
