...
**Tide Here:** the coastal model and place catalogue are embedded in the HTML. Clearing browser storage cannot remove them from a saved copy of that file. Optional forecast history and saved online responses use browser storage; it has download and restore controls. There is no yearly prediction table that runs out: the engine calculates dates from harmonic constants. Coastlines, model accuracy, time-zone rules and browser support can change, so this is not a promise of perpetual accuracy or compatibility.
