...
Tide Here's normal Show tides submission searches Photon first, with a one-second minimum between uncached searches, then falls back to its bundled place catalogue on failure or no match. One exact label/prefix match resolves automatically; county labels are distinct from cities. Ambiguous names show choices. The selected place triggers independent NOAA/CHS catalogue requests and offers up to 12 stations within 150 km. A station within 25 km whose distance is at most 0.6 times the next station's distance is chosen automatically, matching the hosted app's rule; other matches open the station chooser before any forecast is displayed. Failed official requests or no station produce a visibly labelled FES2022 fallback. Local model only, or a browser-reported offline state, bypasses network requests in the normal flow. Coordinates, location, Today, history and deep links all use this path. Manual station and model choices remain in collapsed Data sources and station options. UTC provider events are placed into five local days; the nearby bundled coastline supplies the station time zone, with an explicit UTC fallback where absent. NOAA heights are MLLW; Canadian heights are chart datum; model heights remain mean sea level. CHS unlabeled extrema are sorted/deduplicated and must alternate before high/low labels are inferred. Live official forecasts identify the API rather than claiming WASM calculated their tides.

Searches/catalogues cache for seven days, predictions for six hours, with at most 32 cached responses. Stale responses are used only after a failed refresh and display their original retrieval time and a visible notice. Cache quota failure retains a window-only copy. Cancellation never substitutes a cached response. Forecast history/backups retain provider and datum. The main flow prefers online data; the offline mode and manual source controls remain available without a custom application server.
